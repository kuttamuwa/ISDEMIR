...
class IcmalReportGenerator(object):
    ...
    @staticmethod
    def ada_parsel_merger(df, adanofield='Ada No', parselnofield='Parsel No', mergefield='AdaParsel'):
        df[mergefield] = df[[adanofield, parselnofield]].apply(lambda row: '-'.join(row.values.astype(str)), axis=1)
        return df
    ...
    def execute(self, parameters, messages):
        ...
        if icmal_type == report_choice_list[0]:
            ...
            # number formatting
            df_detail['Ada No'] = df_detail['Ada No'].astype(float).map('{:,.2f}'.format)
            # df_detail['Toplam İnşaat Alanı'] = df_detail['Toplam İnşaat Alanı'].astype(str) \
            #     .apply(lambda x: x.split(".")[0] if x.count(".") else 'Kayıt Yok')
            ...
            # record formatting
            df_detail['Yapım Tarihi'] = [d.strftime('%Y') if not pd.isnull(d) else '' for d in
                                         df_detail['Yapım Tarihi']]
